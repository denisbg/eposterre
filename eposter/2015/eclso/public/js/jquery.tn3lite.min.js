...
(function(f) {
	...
	if (/1\.(0|1|2|3|4)\.(0|1)/.test(f.fn.jquery) || /^1.1/.test(f.fn.jquery) || /^1.2/.test(f.fn.jquery) || /^1.3/.test(f.fn.jquery)) alert("tn3gallery requires jQuery v1.4.2 or later!  You are using v" + f.fn.jquery);
		...
		f.fn.tn3.config = {
			...
			skin: null,
			skinDir: "skins",
			skinDefault: '<div class="tn3-gallery"><div class="tn3-next tn3_v tn3_o"></div><div class="tn3-prev tn3_v tn3_o" style="display:block;"></div><div class="tn3-image"><div class="tn3-text-bg"><div class="tn3-image-title"></div><div class="tn3-image-description"></div></div><div class="tn3-preloader tn3_h tn3_v"><img src="preload.gif"/></div><div class="tn3-timer"></div></div><div class="tn3-controls-bg tn3_rh"><div class="tn3-sep1"></div><div class="tn3-sep2"></div><div class="tn3-sep3"></div></div><div class="tn3-thumbs"></div><div class="tn3-fullscreen"></div><div class="tn3-show-albums"></div><div class="tn3-next-page"></div><div class="tn3-prev-page"></div><div class="tn3-play"></div><div class="tn3-count"></div><div class="tn3-albums"><div class="tn3-inalbums"><div class="tn3-album"></div></div><div class="tn3-albums-next"></div><div class="tn3-albums-prev"></div><div class="tn3-albums-close"></div></div></div>',
			cssID: null
		};
...
(function(f) {
	...
	f.fn.tn3.Gallery.config = {
		...
		prevSlideName:"Previous slide",
		nextSlideMinName:"Next page",
		prevSlideMinName:"Previous page",
	...
	f.fn.tn3.Gallery.prototype = {
			...
			if (!(this.loader.reqs > 0 || this.data.length == 0 || c)) {
				...
				f.each(this.items, function(g, i) {
					...
					case "next":
						...
						i.attr("title", b.t(b.config.nextSlideName));
						break;
					case "prev":
						i.click(function(k) {
							b.show("prev");
						...
						i.attr("title", b.t(b.config.prevSlideName));
						break;
					case "next-page":
						i.click(function() {
							b.items.thumbs && b.thumbnailer.next(true)
						});
						i.attr("title", b.t(b.config.nextSlideMinName));
						break;
					case "prev-page":
						i.click(function() {
							b.items.thumbs && b.thumbnailer.prev(true)
						});
						i.attr("title", b.t(b.config.prevSlideMinName));
						break;
					case "thumbs":
					...
					case "fullscreen":
						...
						i.attr("title", b.t(b.config.maximizeName))
					}
				});
				...
				this.config.isFullScreen && this.onFullResize(f(window).i(), f(window).height());
				this.trigger("init")
			}
		},
		...
		fullscreen: function() {
			if (this.config.isFullScreen) {
				...
				//this.config.fullOnly && this.$c.show(); //on cache les boutons des slides miniatures
				if (this.items.fullscreen) {
					this.items.fullscreen.addClass(this.config.cssID + "-fullscreen-active");
					this.items.fullscreen.attr("title", this.t(this.config.minimizeName))
				}
				this.onFullResize();
				e = this;
				this.trigger("fullscreen", {
		...
		onEscape: function(a) {
			...
			a.keyCode == 39 && nextSlide && e.show("next");
			a.keyCode == 37 && prevSlide && e.show("prev");
		...
		resize: function(a, c) {
			...
			if (this.items.image) {
				...
				if (this.imager){
					if(this.config.isFullScreen){ //maximize
						this.imager.setSize(a, i);
					}else{ //minimize
			...
			/*Plaçage des bouton next et prev*/
			if(this.config.isFullScreen){ //maximize
				imageFull = parseInt($('.blockPage').find('.tn3-full-image').width())/2;
			
				btnNext = parseInt($('.blockPage').find('.tn3-next').css('left').replace('px', ''));
				marge = btnNext + imageFull + 100;
				$('.blockPage').find('.tn3-next').css('left', marge+'px') //bouton next
				btnPrev = parseInt($('.blockPage').find('.tn3-prev').css('right').replace('px', ''));
				marge = btnPrev + imageFull + 100;
				$('.blockPage').find('.tn3-prev').css('right', marge+'px') //bouton prev
			}else{ //minimize
				$('.tn3-next').css('left', '700px') //bouton next
				$('.tn3-prev').css('right', '700px') //bouton prev
			}
			...
			if(this.config.isFullScreen){ //maximize
				...
				if($('.playervideo').size()>0){$('.playervideo').remove();}
				if($('#videos').size()>0)$('#videos').remove();
				$.ajax({
		  			type: "GET",
		  			url: baseUrl+'/index/checkvideo',
		  			data: {abstract:$('#slide').attr('idposter'), slide:$('.tn3-image').children('.tn3-image-in').find('img').attr('src'), width:$('.tn3-in-image').width(), height:$('.tn3-in-image').height()},
		  			success: function(content) {
			  			if(content){
			  				$(document).ready(function(){
				 				 $('.tn3-image').append('<div id="videos"></div>');
				 				 $('#videos').attr("style", $('.tn3-in-image').attr('style')).show();
				 				 $('#videos').append(content);
				 				// $('#videos').css('opacity',null);
				 			 });
			  			}
		  			},
		 			error: function(xhr) {
		...
		center: function() {
			...
			f.each(this.special.v, function(d, b) {
				a = c.items[b];
				a.css("top", ($('.tn3-in-image').height() - a.height()) / 2) //positionnement des boutons
			});
	...
	f.fn.tn3.Imager.prototype = {
		...
		show: function(e) {
			...
			if (this.isInTransition) this.qid = e;
				...
				if (e == "next") {
					prevSlide = true;
					if(this.active > this.data.length-3)nextSlide = false
					if(this.active + 1 < this.data.length){
						e = this.active + 1;
					}else{
						e = 0;
					}
					...
					nextSlide = true;
					if(this.active < 2)prevSlide = false;
					if(this.active > 0){ //déplacement au clavier
						e = this.active -1;
						//prevSlide = true;
					}else{
						e = this.active;
					...
					//e = this.active > 0 ? this.active - 1 : 0;
					this.side = "right"
			...
			$.ajax({
	  			type: "GET",
	  			url: baseUrl+'/index/checkvideo',
	  			data: {abstract:$('#slide').attr('idPoster'), slide:this.data[e].img, width:$('.tn3-in-image').width(), height:$('.tn3-in-image').height()},
	  			success: function(content) {
		  			if(content){
		  				$(document).ready(function(){
			 				 var video = '<div id="videos">'+content+'</div>';
			 				 $('.tn3-image').append(video);
			 				 $('#videos').attr("style", $('.tn3-in-image').attr('style')).show();
			 				// $('#videos').css('opacity',null);
			 			 });
		  			}
		...
		initImage: function(e, a) {
			...
			if(a + 1 >= this.data.length){ //bouton next
				$('.tn3-next').hide() //cache le bouton next si on se trouve sur la derniere slide
				nextSlide = false;
			}else{
				$('.tn3-next').show() //affiche le bouton next si on ne se trouve pas sur la derniere slide
				nextSlide = true;
			}
		
			if(a == 0){ //bouton prev
				$('.tn3-prev').hide() //cache le bouton prev si on se trouve sur la premiere slide
				$('.tn3-prev-page').hide();	
				prevSlide = false;
			}else{
				$('.tn3-prev').show() //affiche le bouton prev si on ne se trouve pas sur la premiere slide
				prevSlide = true;
			}
			
			if(this.data.length < 6){ //bouton next et prev miniatures
				$('.tn3-prev-page').hide();
				$('.tn3-next-page').hide();
			}
...
(function(f) {
	...
	f.fn.tn3.Thumbnailer.prototype = {
		...
		next: function(e) {
			...
			//si le bouton prev est caché, on l'affiche
			if($('.tn3-prev-page').is(':hidden')){ //bouton next et prev miniatures
				$('.tn3-prev-page').show();
			}
		},
		prev: function(e) {
			...
			//si le bouton next est caché, on l'affiche
			if($('.tn3-next-page').is(':hidden')){ //bouton next et prev miniatures
				$('.tn3-next-page').show();
			}
		...
		move: function(e) {
			...
			finSlider = -(this.listSize - this.containerSize) //calcul le décalage pour afficher la fin du slider
			this.$ul.stop();
			this.$ul.animate(a, 300, function(){
				var posSlider = a.left;
				if(parseInt(posSlider) == '0'){
					$('.tn3-prev-page').hide();
				}else if(parseInt(posSlider) == finSlider){
					$('.tn3-next-page').hide();
				}
			})
		},
			...
			if (this.active != -1) {
				...
				if (c + d > this.containerSize || c + d < 0) {
					...
					if(e){
						...
						if($('.blockPage').size() == 0 ){ //si on est pas en plein ecran
							if(this.active >= 5 && $('.tn3-prev-page').is(':hidden'))$('.tn3-prev-page').show(); // si on est a plus de 5 slides, on affiche le bouton precedent des miniatures
							if(this.active < 3 && $('.tn3-prev-page').is(':visible'))$('.tn3-prev-page').hide(); //si on revient vers le debut (3eme slide) et que le bouton precedent est visible, on le cache
						
							finSlider = this.data.length-this.active
							if(finSlider < 4 && $('.tn3-next-page').is(':visible'))$('.tn3-next-page').hide(); //si on va vers la fin (total des slide - 4), et que le bouton suivant est affiché, on le cache
							if(finSlider > 5 && $('.tn3-next-page').is(':hidden'))$('.tn3-next-page').show(); //si on va vers la fin (total des slide - 4), et que le bouton suivant est affiché, on le cache
						}
					}
