...
class Mail extends Zend_Db_Table {
  ...
  protected $_name = 'mail';
    ...
    public function getMails() 
    {
    	$select = $this -> select() 
        		->from('mail')
       			->join('abstract', 'abstract.idAbstract = mail.idAbstract', array('idCases', 'idAbstract', 'idUser', 'title'))
       			->join('profile', 'profile.idCases = abstract.idCases AND profile.idUser = abstract.idUser', array('firstName', 'lastName', 'login'))
       			->where('etat = 0')
